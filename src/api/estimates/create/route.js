async function handler({
  ...
  try {
    ...
    const estimateQuery = sql`
      ...
      RETURNING *
    `;
    queries.push(estimateQuery);

    const [estimateResult] = await sql.transaction([estimateQuery]);
    ...
    const serviceResults = await sql.transaction(serviceQueries);

    return {
      estimate,
      services: serviceResults.flat(),
    };
  } catch (error) {
